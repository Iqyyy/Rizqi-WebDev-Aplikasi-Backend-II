...
const insertBulkUser = async (params) => {
    try {
        ...
        for (let a=0;a<(myArrayList.length-1)/7;a++){
            const query = `insert into praktikan_webdev values ('${myArrayList[a*7]}','${myArrayList[(a*7)+1]}','${myArrayList[(a*7)+2]}','${myArrayList[(a*7)+3]}','${myArrayList[(a*7)+4]}','${myArrayList[(a*7)+5]}')`;
            const result = await databaseQuery(query);
            if (!result) {
